...
def compute_rmse(pred_traj, gt_traj):
    """计算RMSE"""
    return np.sqrt(np.mean([np.sum((np.array(p) - np.array(g)) ** 2)
                           for p, g in zip(pred_traj, gt_traj)]))
...
def format_reward(predict_str: str) -> float:
    ...
    try:
        ...
        # 新增：尝试提取20个数字作为兜底
        nums = re.findall(r"[-+]?[0-9]*\.?[0-9]+", answer_content)
        if len(nums) >= 20:
            nums = [float(x) for x in nums[:20]]
            traj1 = [[nums[i], nums[i+1]] for i in range(0, 10, 2)]
            traj2 = [[nums[i], nums[i+1]] for i in range(10, 20, 2)]
            pred_trajectories = [traj1, traj2]
        else:
            return 0.0
...
def acc_reward(predict_str: str, ground_truth: str) -> tuple:
    try:
        # 提取预测中的 <answer>
        answer_pattern = re.compile(r'<answer>(.*?)</answer>', re.DOTALL)
        answer_match = re.search(answer_pattern, predict_str)
        ...
        try:
            ...
            print(f"acc_reward: 解析预测失败，尝试兜底：{answer_content} -> {e}")
            nums = re.findall(r"[-+]?[0-9]*\.?[0-9]+", answer_content)
            if len(nums) >= 20:
                nums = [float(x) for x in nums[:20]]
                traj1 = [[nums[i], nums[i+1]] for i in range(0, 10, 2)]
        ...
        for pred_traj, gt_traj in zip(pred_trajectories, gt_trajectories):
            ...
            dtw_fd = compute_discrete_frechet(pred_traj, gt_traj)


            rmse_scores.append(rmse)
            hd_scores.append(hd)
            dtw_scores.append(dtw_fd)

            trajectory_score = (rmse + hd + dtw_fd) / 3
            metrics.append(trajectory_score)

        avg_rmse_score = sum(rmse_scores) / len(rmse_scores)
        avg_hd_score = sum(hd_scores) / len(hd_scores)
        avg_dtw_score = sum(dtw_scores) / len(dtw_scores)
        avg_score = sum(metrics) / len(metrics)

        return avg_score, avg_rmse_score, avg_hd_score, avg_dtw_score
...
def compute_score(predict_str: str, ground_truth: str) -> tuple:
    acc_score, rmse_score, hd_score, dtw_score = acc_reward(predict_str, ground_truth)
    total_score = 0.1 * format_reward(predict_str) + 0.9 * acc_score
    return total_score, rmse_score, hd_score, dtw_score
